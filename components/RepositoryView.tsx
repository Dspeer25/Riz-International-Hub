...
type RepoData = Record<number, Folder[]>;
...
export default function RepositoryView() {
  ...
  const deleteFolder = (folderId: string) => {
    setData({
      ...data,
      [selectedMonth]: folders.filter(f => f.id !== folderId),
    });
  ...
  const handleFiles = (fileList: FileList) => {
    ...
    Array.from(fileList).forEach((file) => {
      ...
      reader.onload = (e) => {
        ...
        setData((prev) => {
          const monthFolders = prev[selectedMonth] || [];
          return {
            ...prev,
            [selectedMonth]: monthFolders.map(f =>
              f.id === openFolderId ? { ...f, files: [...f.files, newFile] } : f
            ),
          };
        });
      };
  ...
  const deleteFile = (fileId: string) => {
    ...
    setData({
      ...data,
      [selectedMonth]: folders.map(f =>
        f.id === openFolderId ? { ...f, files: f.files.filter(fl => fl.id !== fileId) } : f
      ),
    });
  };
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2 text-sm">
          <button
            ...
            className={`font-medium ${openFolder ? 'text-[#3d5a80] hover:underline cursor-pointer' : 'text-[#111111]'}`}
          >
          ...
          {openFolder && (
            <>
              ...
              <span className="font-medium text-[#111111]">{openFolder.name}</span>
            </>
          )}
        </div>
        {!openFolder && (
          <button
            ...
            className="px-3 py-1.5 bg-[#1a2744] text-white text-sm font-medium rounded-lg hover:bg-[#243356] transition-colors inline-flex items-center gap-1.5"
          >
      ...
      {creatingFolder && !openFolder && (
        <div className="flex items-center gap-2 mb-4 bg-white border border-gray-200 rounded-lg p-3">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#3d5a80" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          <input
            ...
            className="flex-1 text-sm text-[#111111] outline-none placeholder:text-[#999999]"
            autoFocus
          />
          <button onClick={createFolder} className="text-green-600 hover:text-green-700">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <polyline points="20 6 9 17 4 12" />
            </svg>
          </button>
          <button onClick={() => { setCreatingFolder(false); setNewFolderName(''); }} className="text-[#999999] hover:text-[#666666]">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
              {folders.map((folder) => (
                ...
                >
                  ...
                  <p className="text-xs text-[#999999] mt-0.5">{folder.files.length} file{folder.files.length !== 1 ? 's' : ''}</p>
                  <button
                    onClick={(e) => { e.stopPropagation(); deleteFolder(folder.id); }}
                    className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity text-[#999999] hover:text-red-500 p-1"
                    title="Delete folder"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div>
          ...
          <div
            ...
            className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors mb-6 ${
              isDragging ? 'border-[#3d5a80] bg-blue-50' : 'border-gray-200 hover:border-[#3d5a80] hover:bg-[#f8f9fb]'
            }`}
          >
            <div className="text-[#999999] mb-2 flex justify-center">
              <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          {openFolder.files.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-sm text-[#999999]">No files in this folder yet</p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[#999999]">
                      {formatFileSize(file.size)} · {new Date(file.uploaded_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                    </p>
                  </div>
                  <button
                    onClick={() => downloadFile(file)}
                    className="text-[#3d5a80] hover:text-[#1a2744] transition-colors p-1.5"
                    title="Download"
                  >
                  ...
                  <button
                    ...
                    className="text-[#999999] hover:text-red-500 transition-colors p-1.5"
                    title="Delete"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M3 6h18" /><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6" /><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2" />
